...
-- This is the implementation of AbstractCurry as implemented in the Curry frontend.
-- AbstractCurry is used as the representation of Curry in Splices. We reuse the 
-- implementation of AbstractCurry as we don't want to reinvent the wheel.
--
-- By adding AbstractCurry to the LanguageExtension we can now just use the type check
-- for "forcing AbstractCurry" in Splices. Also we don't need to teach the frontend to 
-- compile AbstractCurry to FlatCurry anymore. This is basically doing the same thing.
...
data CLiteral
  = CIntc    Int -- ^ Int literal
  | CFloatc  Float  -- ^ Float literal
